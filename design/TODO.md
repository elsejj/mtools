...
### 3.1 页面宏观布局
- [ ] 顶部导航栏：Logo、候选推荐工具条（带 `Alt+数字` 快捷键徽标）、设置与历史入口
- [ ] 左侧边栏：工具分类列表、搜索过滤框、快捷添加自定义工具按钮
- [ ] 主工作区：左右分栏（输入原稿 vs 处理结果）

### 3.2 高效结果渲染 (零数据计算，纯展示)
- [ ] 渲染 Rust 端已格式化好的 Pretty JSON（代码语法高亮与折叠）
- [ ] 渲染多模态图片预览组件
- [ ] 顶部“解码溯源提示条”（如：“💡 已自动通过 Base64 解码，点击可查看原始 Base64 内容”）

### 3.3 底部交互栏与后置反馈
- [ ] 一键复制结果到剪贴板
- [ ] “回贴源软件”按键（触发 `simulate_paste`）
- [ ] 执行耗时与状态指示器
- [ ] 后置处理结果反馈（如保存文件后的物理路径链接，点击调用 `show_in_folder`）
