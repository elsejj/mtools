...
### 4.1 内置代码处理型工具
- [x] 验证 JSON 格式化工具全链路跑通
- [x] 验证 URL 编解码 / 参数解析工具
- [x] 验证 JWT 结构展示工具
- [x] 验证时间戳互转工具

### 4.2 大模型客户端与流式输出
- [x] 封装统一的 OpenAI 兼容客户端
- [x] 支持纯文本与图文多模态动态拼装 (Content Parts)
- [x] 支持 SSE 流式打字机渲染与一键中断生成 (`AbortController`)
- [x] 验证 OCR 文字提取工具
- [x] 验证多语言翻译与文本润色工具

### 4.3 外部 CLI 命令工具接入
- [x] 联调调用系统 CLI（如 `jq`, `python` 等）输出实时回显

### 4.4 后置处理流水线集成
- [x] 验证流式/非流式执行完成后，自动触发后置动作（自动复制 / 自动按时间戳存盘）
...
### 5.1 自定义工具向导
- [x] 新增/编辑工具弹窗：支持代码型、AI 型、CLI 型向导式表单
- [x] 规则匹配实时验证面板（输入样例即可实时查看匹配分数）
- [x] 配置专属后置处理目录与行为
- [x] 工具的停用、排序、删除与 JSON 导出/导入
...
### 5.3 系统设置中心
- [x] AI 服务商池管理（添加多个 Provider、配置 BaseURL、API Key、测试连接）
- [x] 设定全局默认模型
- [x] 主题切换（Light / Dark / Follow System）
- [x] 图片缓存统计展示（大小/张数）与一键清理按钮
...
### 6.1 端到端典型场景测试
- [x] **场景 1（普通文本与纠偏）**：在外部选中乱序 JSON，快捷键启动，验证是否自动由非相关工具平滑纠偏跳转至 JSON 格式化工具，并以 Pretty 格式呈现。
- [x] **场景 2（Base64 还原与升格）**：在外部选中 Base64 图片，快捷键启动，验证前置解码器是否检测到 PNG 魔数并自动升格为图片，直接送入 OCR / 图片工具。
- [x] **场景 3（后置处理）**：配置 OCR 工具后置处理为存盘，测试识别完成后是否在专属目录生成 `YYYY-MM-DD_HH-mm-ss.txt` 文件并支持一键打开。
- [x] **场景 4（窗口位置记忆）**：移动与缩放窗口后关闭，外部重新快捷键唤起，验证窗口是否精确在上次位置与大小还原。

### 6.2 编译与代码检查
- [x] 运行 `bun run check:f` 确保前端 TypeScript 类型检查零错误
- [x] 运行 `bun run check:m` 确保 Rust 后端编译通过、无 warning
