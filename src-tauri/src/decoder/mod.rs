...
impl DecoderPipeline {
    ...
    fn try_decode_hex(input: &str) -> Option<DecodedOutput> {
        let clean_hex = if let Some(stripped) = input.strip_prefix("0x").or_else(|| input.strip_prefix("0X")) {
            stripped.trim()
        } else {
            input
        };
        ...
        if clean_hex.len() < 8 || clean_hex.len() % 2 != 0 || !clean_hex.chars().all(|c| c.is_ascii_hexdigit()) {
            return None;
        }
...
mod tests {
    ...
    fn test_png_magic_upgrade() {
        ...
        match DecoderPipeline::decode(png_base64) {
            DecodedOutput::Image { mime_type, trace, .. } => {
