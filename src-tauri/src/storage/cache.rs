...
pub fn save_image_cache(data_dir: &Path, bytes: &[u8]) -> Result<String, String> {
    ...
    fs::create_dir_all(&target_dir).map_err(|e| format!("Failed to create image cache directory: {}", e))?;
...
pub fn cleanup_cache(data_dir: &Path, max_age_days: Option<u32>, force_all: bool) -> Result<u64, String> {
    let images_dir = data_dir.join("cache").join("images");
    if !images_dir.exists() {
        return Ok(0);
    }
