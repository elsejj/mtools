...
pub fn save_tool_config(conn: &Connection, tool: &Value) -> Result<(), String> {
    let id = tool.get("id").and_then(|v| v.as_str()).ok_or("Missing tool id")?;
    let name = tool.get("name").and_then(|v| v.as_str()).unwrap_or("Untitled Tool");
    let tool_type = tool.get("type").and_then(|v| v.as_str()).unwrap_or("code");
    let is_custom = if tool.get("isCustom").and_then(|v| v.as_bool()).unwrap_or(false) { 1 } else { 0 };
    let enabled = if tool.get("enabled").and_then(|v| v.as_bool()).unwrap_or(true) { 1 } else { 0 };
...
pub fn delete_tool_config(conn: &Connection, tool_id: &str) -> Result<(), String> {
    conn.execute("DELETE FROM tools_config WHERE id = ? AND is_custom = 1", params![tool_id])
